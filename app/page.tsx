    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
      <div className="flex flex-col md:flex-row gap-8 items-center max-w-4xl w-full">
        ...
        <div className="flex-1 space-y-6 w-full max-w-xs">
          ...
          <button onClick={saveAsJpg} className="w-full bg-red-800 text-white py-4 rounded-xl font-bold shadow-xl hover:bg-red-900 active:scale-95">SAVE AS JPG</button>
        </div>
      </div>

      {showUploadOption && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full space-y-4 text-center">
            <h3 className="text-xl font-bold text-gray-800">เลือกช่องที่ {currentSlot + 1}</h3>
            <button onClick={startCamera} className="w-full py-4 bg-red-800 text-white rounded-xl font-bold hover:bg-red-900">📸 ถ่ายภาพ</button>
            <button onClick={() => fileInputRef.current?.click()} className="w-full py-4 bg-gray-100 text-gray-700 rounded-xl font-bold">📂 อัปโหลดรูปภาพ</button>
      ...
      {isCameraOpen && (
        <div className="fixed inset-0 bg-black z-[60] flex flex-col items-center justify-center p-4">
          <div className="relative w-full max-w-md aspect-[3/4] bg-gray-900 rounded-2xl overflow-hidden border-4 border-red-800">
            <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover" style={{ filter: filter }} />
          </div>
          <div className="mt-10 flex items-center gap-10">
            <button onClick={stopCamera} className="text-white">ยกเลิก</button>
            <button onClick={takePhoto} className="w-20 h-20 bg-white rounded-full border-8 border-red-800 active:scale-90 transition-all"></button>
